  ...
  module.exports = Type = (function() {
    function Type(type) {
      if (typeof type === 'string') {
        if (indexOf.call(Type.TYPE, type) >= 0 && type !== Type.TYPE.ARRAY && type !== Type.TYPE.OBJECT) {
    ...
    Type.prototype.write = function(value, data, path) {
      var field, i, k, len, ref, results, subValue, subpath;
      ...
      i = 0;
      results = [];
      for (len = k = 0, ref = this.fields.length; 0 <= ref ? k < ref : k > ref; len = 0 <= ref ? ++k : --k) {
        field = this.fields[i++];
        ...
        results.push(this._writeArray(subValue, data, subpath, field.type));
      }
      return results;
    };
    ...
    Type.prototype._writeArray = function(value, data, path, type) {
      ...
      for (i = k = 0, ref = len; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
        results.push(type.write(value[i], data, path + '.' + i));
      }
      return results;
    };
    ...
    Type.prototype._readArray = function(type, state) {
      ...
      for (j = k = 0, ref = arr.length; 0 <= ref ? k < ref : k > ref; j = 0 <= ref ? ++k : --k) {
